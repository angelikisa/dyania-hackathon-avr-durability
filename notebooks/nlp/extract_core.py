...
# ---------------------------------------------------------------------------
# Three supported hemodynamic template families:
...
AR_GRADE_PATTERN = re.compile(
    # 'svr' (dictation shorthand for "severe") is included in the grade alternation
...
def nearest_valve_context(text: str, pos: int, window: int = 400):
    ...
    Echo reports often narrate one valve at a time in prose, so
    'closest keyword wins' is the selected heuristic rather
...
# "History: ..." narrative paragraph containing clinical detail.
# Only a match that is BOTH under a PMH/PSH/problem-list
# section header AND sitting alone on a short, unelaborated line is treated
# as nonspecific coded history because the phrase alone cannot distinguish
# SVD from PVL, endocarditis, or thrombosis.
...
def extract_redo(text: str):
    ...
    for h in hits:
        if h.get("start") is not None:
            # A redo mention introduced by a PMH/PSH frame is restating a known prior
...
def extract_implicit_new_tavr(text: str):
    ...
    'valve-in-valve' wording. This is only meaningful as a
...
def extract_svd_explicit(text: str):
    ...
    prostheses, immediately followed by the disambiguating
