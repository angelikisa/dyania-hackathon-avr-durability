...
# development examples: a same-clause causal link is close to the trigger,
# while an unrelated remote-history mention is substantially farther away.
...
def _trigger_near_exclusion_same_line(text: str, exclusions: dict, trigger_start: int, trigger_end: int) -> str:
    ...
    exclusion check below: separate coded-problem-list lines must not be
    treated as causally connected, while terms within one narrative sentence
    may be related. The redo/ViV/SVD-explicit exclusion checks don't need this
    extra guard because their own narrative-keyword requirement already
    anchors the proximity heuristic to a narrative clinical statement; a bare
...
def _restates_index_note_text(text: str, hit: dict, index_note_texts, window: int = 120) -> bool:
    ...
    copy-forward of the patient's own dated implant-event note. Copy-forward
    text is a common EHR pattern and does not represent a new event."""
...
def run_pipeline(notes_source):
    ...
    for pid, sub in df.groupby("Profile Key"):
        ...
        for idx, row in sub.iterrows():
            ...
            # not post-implant surveillance of the new prosthesis.
            ...
            # an identified source of false-positive HVD staging).
            ...
            # by a non-structural cause nearby. We
