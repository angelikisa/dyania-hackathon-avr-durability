...
SAMPLE_NOTES_PATH = ROOT / "samples" / "synthetic_valve_notes.txt"
EVIDENCE_COLUMNS = (
...
def process_notes_file(file_bytes: bytes, filename: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    from notebooks.nlp.pipeline import run_pipeline
...
with st.expander("How reliable is this estimate?"):
    st.markdown(
        ...
        '<div><strong>11</strong><span>valve-failure events informed the model</span></div>'
